...
function MobileResponsiveView({driverData,...rest}) {
  ...
  <Tbody>
    {displayedData.map((item) => (
      ...
      <Tr key={item.id    }   as="tr" borderBottom= {{base: "1px solid orange"  }}    >
        <Td   borderBottom=".5px solid  orange" style={{ boxSizing: "border-box" }}  >
            <Text className=" mb-4 font-bold"> Name: {item.name}</Text>
            <Text className=" text-gray-700">License Expiry: {item.DateOflicense}</Text>
        </Td>
      <Td   borderBottom=".5px solid " style={{ boxSizing: "border-box" }}  className=" text-orange-500" p={0}><Link >View</Link></Td>
      </Tr>
    ))}
  </Tbody>
  <tfoot>
    <Tr>
      ...
      <Flex justifyContent="flex-end">
        ...
        <HStack spacing={2} justify="center">
          ...
          >
          <Text className={currentPage===1?" bg-slate-500 p-3":"  p-3  "}>< IoIosArrowBack fontSize={25} /></Text>
          </Button>
          {getPageNumbersToShow  ().map((pageNumber) => (
          <Button  bg={"white"}    className={pageNumber===currentPage?"text-orange-500 btn border border-1 border-solid border-red-500    "  :" hover:border-red-500 hover:text-red-500 border-2   rounded-md  p-3 font-bold   text-xl text-gray-700 border-red    "}
            key={pageNumber} 
        ...
        {pageCount > maxButtonsToShow && (
          ...
          <Text className={currentPage===pageCount?"  bg-slate-500 p-3":" p-3"}>< IoIosArrowForward fontSize={20} /></Text>
