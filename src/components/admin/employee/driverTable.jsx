...
const DriverTable = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text) => (
        <Typography.Text>{moment(new Date(text)).format(import.meta.env.VITE_DATE_DISPLAY_FORMAT)}</Typography.Text>
      ),
    },
    {
      title: 'Action',
      key: 'action',
      render: (_, record) => (
        <Button type="link" onClick={onViewButton(record.id)}>
          View Details
        </Button>
      ),
    },
  ...
  const listRender = (record) => (
    <>
      ...
      <Typography.Text>
        Licence expiry: {moment(new Date(record.licence_expiration)).format(import.meta.env.VITE_DATE_DISPLAY_FORMAT)}
      </Typography.Text>
    </>
  );
  return (
